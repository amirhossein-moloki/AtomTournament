...
from .models import Tournament, Game, Match
...
class TournamentAPITest(APITestCase):
    def setUp(self):
        ...
        self.user1 = User.objects.create_user(username='user1', password='password', phone_number='+12125552361')
        self.user2 = User.objects.create_user(username='user2', password='password', phone_number='+12125552362')
        from wallet.models import Wallet
        Wallet.objects.create(user=self.user1, total_balance=100, withdrawable_balance=50)
        Wallet.objects.create(user=self.user2, total_balance=100, withdrawable_balance=50)
    ...
    def test_generate_matches(self):
        ...
        self.user1.save()
        url = reverse('tournament-generate-matches', kwargs={'pk': self.individual_tournament.pk})
        response = self.client.post(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_distribute_prizes(self):
        ...
        self.user1.is_staff = True
        self.user1.save()
        url = reverse('tournament-distribute-prizes', kwargs={'pk': self.individual_tournament.pk})
        response = self.client.post(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
