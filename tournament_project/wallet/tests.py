...
from .services import distribute_prize, pay_entry_fee, update_wallet_balance
...
class WalletServiceTest(TestCase):
    def setUp(self):
        ...
        self.wallet = Wallet.objects.create(
            user=self.user, total_balance=100, withdrawable_balance=50
        )
    ...
    def test_distribute_prize_individual(self):
        ...
        self.wallet.refresh_from_db()
        self.assertEqual(
            self.wallet.total_balance, Decimal("120.00")
        )  # 100 (initial) + 20 (prize)
        self.assertEqual(
            self.wallet.withdrawable_balance, Decimal("70.00")
        )  # 50 (initial) + 20 (prize)
    ...
    def test_distribute_prize_team(self):
        ...
        user3 = User.objects.create_user(
            username="user3", password="password", phone_number="+12125552370"
        )
        user4 = User.objects.create_user(
            username="user4", password="password", phone_number="+12125552371"
        )
        ...
        wallet2 = Wallet.objects.create(
            user=user2, total_balance=100, withdrawable_balance=50
        )
