...
class ReportingAPITests(TestCase):
    ...
    def test_statistics_endpoint_permissions(self):
        """Test that only admin users can access the statistics endpoint."""
        self.client.force_authenticate(user=self.normal_user)
        response = self.client.get('/api/reporting/statistics/')
        self.assertEqual(response.status_code, 403)

    def test_statistics_endpoint_data(self):
        """Test the statistics endpoint data."""
