...
from django_ratelimit.core import (_get_ip, _split_rate, get_usage,
                                   is_ratelimited)
...
class RatelimitTests(TestCase):
    ...
    def test_ratelimit_custom_exception_class(self):
        ...
        with self.settings(RATELIMIT_EXCEPTION_CLASS=CustomRatelimitedException):
            req = rf.get("")
            assert not view(req)
            with self.assertRaises(CustomRatelimitedException):
                view(req)
    ...
    def test_callable_rate_import(self):
        ...
        @ratelimit(
            key="user_or_ip", rate="django_ratelimit.tests.callable_rate", block=False
        )
        def view(request):
            return request.limited
    ...
    def test_sorted_methods(self):
        ...
        @ratelimit(rate="1/m", key="ip", method=["GET", "POST"], group="a", block=False)
        def get_post(request):
            return request.limited

        @ratelimit(rate="1/m", key="ip", method=["POST", "GET"], group="a", block=False)
...
class RatelimitCBVTests(TestCase):
    ...
    def test_methods_counted_separately(self):
        class TestView(View):
            @method_decorator(
                ratelimit(key="ip", rate="1/m", method="GET", block=False)
            )
            def get(self, request):
                return request.limited
